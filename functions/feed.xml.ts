...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  const siteDomain = env.SITE_DOMAIN || requestUrl.hostname.replace(/^www\./, '');
  const siteUrl = env.SITE_URL || `${requestUrl.protocol}//${siteDomain}`;
...
<rss version="2.0">
  <channel>
    <title>${siteDomain} - Edukasi &amp; Informasi Artikel</title>
    <link>${siteUrl}</link>
    <description>Portal artikel berita &amp; edukasi terpercaya.</description>
