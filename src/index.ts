...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    const siteUrl = env.SITE_URL || `${url.protocol}//${siteDomain}`;
...
<rss version="2.0">
  <channel>
    <title>${siteDomain} - Portal Artikel</title>
    <link>${siteUrl}</link>
    ...
    return new Response(`Cloudflare Worker Edge Engine Active (${siteDomain})`, { status: 200 });
  },
};
