...
import { Heart, ShieldCheck, Zap, Menu, X, UserCheck, FileText, Rss, Baby, Sparkles, BookOpen, Sun, Moon, Monitor } from 'lucide-react';
...
export default function Header({ currentView, onNavigate, currentUser, onLogout, siteConfig }: HeaderProps) {
  ...
  const siteName = siteConfig?.site_name || siteConfig?.site_domain || 'Edge Blog';
  const siteTagline = siteConfig?.site_tagline || 'PORTAL EDUKASI POLA ASUH & GIZI ANAK';
  const currentThemeMode = siteConfig?.theme_mode || 'light';

  const handleToggleThemeMode = () => {
    let nextMode: 'light' | 'dark' | 'auto' = 'dark';
    if (currentThemeMode === 'light') nextMode = 'dark';
    else if (currentThemeMode === 'dark') nextMode = 'auto';
    else nextMode = 'light';

    applyTheme({ ...siteConfig, theme_mode: nextMode });

    const cached = localStorage.getItem('parenting_site_config');
    if (cached) {
      try {
        const parsed = JSON.parse(cached);
        parsed.theme_mode = nextMode;
        localStorage.setItem('parenting_site_config', JSON.stringify(parsed));
      } catch (e) {}
    }
  };
          ...
          <nav className="hidden md:flex items-center gap-1">
            ...
            {siteConfig?.enable_theme_toggle !== false && (
              <button
                ...
                className="p-2 rounded-xl text-slate-600 dark:text-slate-300 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 transition-all flex items-center gap-1.5 text-xs font-semibold cursor-pointer"
                title={`Mode Tampilan: ${currentThemeMode.toUpperCase()} (Klik untuk ganti Bright / Dark / Auto)`}
              >
                {currentThemeMode === 'light' && <Sun className="w-4 h-4 text-amber-500" />}
                {currentThemeMode === 'dark' && <Moon className="w-4 h-4 text-indigo-400" />}
                {currentThemeMode === 'auto' && <Monitor className="w-4 h-4 text-emerald-500" />}
                <span className="hidden lg:inline capitalize text-[11px] font-bold">{currentThemeMode}</span>
              </button>
            )}
