...
export default function Footer({ siteConfig }: FooterProps) {
  const siteName = siteConfig?.site_name || siteConfig?.site_domain || 'Modern Edge Blog';
  const aboutText = siteConfig?.footer_about_text || 'Menghadirkan bacaan berkualitas seputar dunia pengasuhan anak, kesehatan keluarga, dan pendidikan anak usia dini.';
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              {siteConfig?.footer_badge_1 !== '' && (
                <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-slate-800 border border-slate-700 text-xs text-rose-300 font-medium">
                  <Zap className="w-3 h-3 text-amber-400" /> {siteConfig?.footer_badge_1 ?? 'Cloudflare Pages Edge'}
                </span>
              )}
              {siteConfig?.footer_badge_2 !== '' && (
                <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-slate-800 border border-slate-700 text-xs text-sky-300 font-medium">
                  <Database className="w-3 h-3 text-sky-400" /> {siteConfig?.footer_badge_2 ?? 'Cloudflare D1 SQLite'}
                </span>
              )}
              {siteConfig?.footer_badge_3 !== '' && (
                <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-slate-800 border border-slate-700 text-xs text-purple-300 font-medium">
                  <GitBranch className="w-3 h-3 text-purple-400" /> {siteConfig?.footer_badge_3 ?? 'GitHub REST Storage'}
                </span>
              )}
            </div>
          </div>
            ...
            <ul className="space-y-2.5 text-sm">
              ...
              <li>
                <span className="text-slate-400 inline-flex items-center gap-1">
                  <span>{siteConfig?.footer_autolink_label ?? 'Auto-Linking Engine On-Page'}</span>
