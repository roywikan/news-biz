  ...
  const handleLogin = async (email: string, pass: string, turnstileToken?: string): Promise<boolean> => {
    try {
      ...
      if (res.ok && contentType && contentType.includes('application/json')) {
        ...
        if (errData?.error) {
          console.error('Login error message:', errData.error);
        }
      }
    } catch (err) {
    ...
    // Client-side fallback (e.g. for static preview)
    const lowerEmail = email.toLowerCase();
    if ((lowerEmail.startsWith('admin@') || lowerEmail.includes('admin')) && (pass === 'admin123' || pass === 'admin')) {
      const adminUser = { ...INITIAL_USERS[0], email: lowerEmail };
      setCurrentUser(adminUser);
      localStorage.setItem('parenting_user', JSON.stringify(adminUser));
      return true;
    } else if ((lowerEmail.startsWith('penulis@') || lowerEmail.startsWith('writer@') || lowerEmail.includes('writer')) && (pass === 'writer123' || pass === 'writer')) {
      const writerUser = { ...INITIAL_USERS[1], email: lowerEmail };
