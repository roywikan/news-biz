...
export default function ArticleDetailView({
  ...
  const activeDomain = getDerivedDomain(siteConfig?.site_domain);
  const articleUrl = typeof window !== 'undefined' ? `${window.location.origin}/baca/${post.slug}` : `https://${activeDomain}/baca/${post.slug}`;
  ...
  return (
    <article className="max-w-4xl mx-auto space-y-8 pb-16">
      <SEOHelper
        title={`${post.title} | ${siteConfig?.site_name || activeDomain}`}
