...
import { 
  ...
  LogOut, Globe, Palette, Layout, MessageSquare, Sun, Moon, Monitor, Type
} from 'lucide-react';
import { generateSlug } from '../lib/autolink';
import RichPostEditor from '../components/RichPostEditor';
import TurnstileWidget from '../components/TurnstileWidget';
import { COLOR_PALETTES, FONT_SCALE_OPTIONS, THEME_MODE_OPTIONS, applyTheme } from '../lib/theme';
...
export default function AdminPortal({
  ...
  const [cfgSiteName, setCfgSiteName] = useState(siteConfig?.site_name || 'Parenting Media');
  const [cfgSiteDomain, setCfgSiteDomain] = useState(siteConfig?.site_domain || '');
  ...
  const [cfgThemeMode, setCfgThemeMode] = useState<'light' | 'dark' | 'auto'>(siteConfig?.theme_mode || 'light');
  const [cfgThemePalette, setCfgThemePalette] = useState<string>(siteConfig?.theme_palette || 'rose-coral');
  const [cfgFontSizeScale, setCfgFontSizeScale] = useState<'compact' | 'standard' | 'spacious'>(siteConfig?.font_size_scale || 'standard');

  const handleModeChange = (mode: 'light' | 'dark' | 'auto') => {
    setCfgThemeMode(mode);
    applyTheme({
      theme_mode: mode,
      theme_palette: cfgThemePalette,
      font_size_scale: cfgFontSizeScale,
    });
  };

  const handlePaletteChange = (paletteId: string) => {
    setCfgThemePalette(paletteId);
    applyTheme({
      theme_mode: cfgThemeMode,
      theme_palette: paletteId,
      font_size_scale: cfgFontSizeScale,
    });
  };

  const handleScaleChange = (scale: 'compact' | 'standard' | 'spacious') => {
    setCfgFontSizeScale(scale);
    applyTheme({
      theme_mode: cfgThemeMode,
      theme_palette: cfgThemePalette,
  ...
  // Customizable Badges & Tech Labels State
  const [cfgHeaderBadgeText, setCfgHeaderBadgeText] = useState(siteConfig?.header_badge_text ?? 'Cloudflare D1 Edge Engine');
  const [cfgHeroBadgeText, setCfgHeroBadgeText] = useState(siteConfig?.hero_badge_text ?? 'Cloudflare D1 Edge Architecture • TTFB < 20ms');
  const [cfgAutolinkTickerLabel, setCfgAutolinkTickerLabel] = useState(siteConfig?.autolink_ticker_label ?? 'Kata Kunci SEO Terpopuler:');
  const [cfgFooterBadge1, setCfgFooterBadge1] = useState(siteConfig?.footer_badge_1 ?? 'Cloudflare Pages Edge');
  const [cfgFooterBadge2, setCfgFooterBadge2] = useState(siteConfig?.footer_badge_2 ?? 'Cloudflare D1 SQLite');
  const [cfgFooterBadge3, setCfgFooterBadge3] = useState(siteConfig?.footer_badge_3 ?? 'GitHub REST Storage');
  const [cfgFooterAutolinkLabel, setCfgFooterAutolinkLabel] = useState(siteConfig?.footer_autolink_label ?? 'Auto-Linking Engine On-Page');
  ...
  useEffect(() => {
    if (siteConfig) {
      ...
      setCfgSiteDomain(siteConfig.site_domain || '');
      ...
      setCfgThemeMode(siteConfig.theme_mode || 'light');
      setCfgThemePalette(siteConfig.theme_palette || 'rose-coral');
      setCfgFontSizeScale(siteConfig.font_size_scale || 'standard');

      setCfgHeaderBadgeText(siteConfig.header_badge_text ?? 'Cloudflare D1 Edge Engine');
      setCfgHeroBadgeText(siteConfig.hero_badge_text ?? 'Cloudflare D1 Edge Architecture • TTFB < 20ms');
      setCfgAutolinkTickerLabel(siteConfig.autolink_ticker_label ?? 'Kata Kunci SEO Terpopuler:');
      setCfgFooterBadge1(siteConfig.footer_badge_1 ?? 'Cloudflare Pages Edge');
      setCfgFooterBadge2(siteConfig.footer_badge_2 ?? 'Cloudflare D1 SQLite');
      setCfgFooterBadge3(siteConfig.footer_badge_3 ?? 'GitHub REST Storage');
      setCfgFooterAutolinkLabel(siteConfig.footer_autolink_label ?? 'Auto-Linking Engine On-Page');
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    if (!success) {
      setLoginError('Email atau password tidak terdaftar / verifikasi Turnstile gagal.');
    }
  };
  ...
  const handleSaveConfigSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updatedCfg: SiteConfig = {
        ...
        site_domain: cfgSiteDomain.trim(),
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 border border-slate-200 dark:border-slate-800 shadow-xl space-y-6">
          ...
          <form onSubmit={handleLoginSubmit} className="space-y-4">
            ...
            <div className="pt-2">
              ...
              <TurnstileWidget
                siteKey="0x4AAAAAAEOu5hMPB-BIYka8"
          ...
          <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-6">
            ...
            <div className="space-y-4">
              <h4 className="text-xs font-bold text-rose-600 dark:text-rose-400 uppercase tracking-wider flex items-center gap-1.5">
                ...
                <span>1. Identitas Website, Domain Name & Header</span>
              </h4>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">
                    Custom Domain Name (site_domain)
                  </label>
                  <input
                    type="text"
                    value={cfgSiteDomain}
                    onChange={(e) => setCfgSiteDomain(e.target.value)}
                    placeholder={typeof window !== 'undefined' ? window.location.hostname : 'domainanda.com'}
                    className="w-full px-3.5 py-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-xs font-semibold font-mono text-rose-600 dark:text-rose-400 focus:ring-2 focus:ring-rose-500"
                  />
                  <span className="text-[10px] text-slate-500 block mt-1">
                    Kosongkan untuk otomatis mendeteksi dari DNS / Host browser ({typeof window !== 'undefined' ? window.location.hostname : 'domainanda.com'})
                  </span>
                </div>
            ...
            {/* SECTION 2: TAMPILAN, PALET WARNA & TIPOGRAFI (THEME ENGINE) */}
            <div className="space-y-6 pt-4 border-t border-slate-100 dark:border-slate-800">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <h4 className="text-xs font-bold text-rose-600 dark:text-rose-400 uppercase tracking-wider flex items-center gap-1.5">
                  <Palette className="w-4 h-4" />
                  <span>2. Tampilan, Palet Warna & Tipografi Responsive (10 Theme Presets)</span>
                </h4>
                <span className="text-[11px] font-semibold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/60 px-2.5 py-0.5 rounded-full border border-emerald-200 dark:border-emerald-800 flex items-center gap-1 w-fit">
                  <Sparkles className="w-3 h-3" /> Live Realtime Preview Active
                </span>
              </div>

              {/* SUB-SECTION 1: MODE TAMPILAN (LIGHT, DARK, AUTO) */}
              <div className="space-y-2">
                <label className="block text-xs font-bold text-slate-700 dark:text-slate-300">
                  Mode Tampilan Website (Default: Bright Mode)
                </label>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  {THEME_MODE_OPTIONS.map((opt) => {
                    const isSelected = cfgThemeMode === opt.id;
                    return (
                      <button
                        key={opt.id}
                        type="button"
                        ...
                        className={`p-3.5 rounded-2xl border text-left transition-all flex flex-col justify-between gap-2 cursor-pointer ${
                          isSelected
                            ...
                            : 'border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950 text-slate-700 dark:text-slate-300 hover:border-slate-300'
                        }`}
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2 font-bold text-xs">
                            {opt.id === 'light' && <Sun className="w-4 h-4 text-amber-500" />}
                            {opt.id === 'dark' && <Moon className="w-4 h-4 text-indigo-400" />}
                            {opt.id === 'auto' && <Monitor className="w-4 h-4 text-emerald-500" />}
                            <span>{opt.name}</span>
                          </div>
                          {isSelected && <Check className="w-4 h-4 text-rose-600 dark:text-rose-400 font-bold" />}
                        </div>
                        <p className="text-[11px] text-slate-500 dark:text-slate-400 leading-relaxed">{opt.description}</p>
                      </button>
                    );
                  })}
                </div>
              ...
              {/* SUB-SECTION 2: RESPONSIVE FONT SCALE */}
              <div className="space-y-2">
                <label className="block text-xs font-bold text-slate-700 dark:text-slate-300">
                  Skala Ukuran Font Responsive (Responsive Typography Scale)
                </label>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  {FONT_SCALE_OPTIONS.map((s) => {
                    const isSelected = cfgFontSizeScale === s.id;
                    return (
                      <button
                        key={s.id}
                        type="button"
                        onClick={() => handleScaleChange(s.id as any)}
                        className={`p-3.5 rounded-2xl border text-left transition-all flex flex-col justify-between gap-1 cursor-pointer ${
                          isSelected
                            ? 'border-rose-500 ring-2 ring-rose-500/30 bg-rose-50/50 dark:bg-rose-950/30'
              ...
              {/* SUB-SECTION 3: 10 PALET WARNA MODERN */}
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300">
                    Pilih Preset Palet Warna Website (10 Palet Peruntukan Domain Webmaster)
                  </label>
                  <span className="text-[10px] text-slate-500">Klik palet untuk preview langsung</span>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
                  {COLOR_PALETTES.map((pal) => {
                    const isSelected = cfgThemePalette === pal.id;
                    return (
                      <button
                        key={pal.id}
                        type="button"
                        onClick={() => handlePaletteChange(pal.id)}
                        className={`p-3.5 rounded-2xl border text-left transition-all flex flex-col justify-between gap-2.5 relative cursor-pointer ${
                          isSelected
                            ? 'border-rose-500 ring-2 ring-rose-500/40 bg-white dark:bg-slate-900 shadow-md'
                            : 'border-slate-200 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-950 hover:border-slate-300 hover:bg-white dark:hover:bg-slate-900'
                        }`}
                      >
                        ...
                        <div className="flex items-center justify-between w-full">
                          <div className="flex items-center -space-x-1.5">
                            <div
                              ...
                              style={{ backgroundColor: pal.swatchColors[0] }}
                            />
                            <div
                              className="w-5 h-5 rounded-full border-2 border-white dark:border-slate-900 shadow-sm"
                              style={{ backgroundColor: pal.swatchColors[1] }}
                            />
                            <div
                              className="w-5 h-5 rounded-full border-2 border-white dark:border-slate-900 shadow-sm"
                              style={{ backgroundColor: pal.swatchColors[2] }}
                            />
                          </div>

                          {isSelected && (
                            <span className="w-5 h-5 rounded-full bg-rose-600 text-white flex items-center justify-center shadow-sm">
                        ...
                        <div>
                          ...
                          <div className="text-[10px] font-semibold text-rose-600 dark:text-rose-400 tracking-wide mt-0.5">
                            {pal.category}
                          </div>
                        </div>
            ...
            {/* SECTION 4: BADGES & CUSTOM LABELS */}
            <div className="space-y-4 pt-4 border-t border-slate-100 dark:border-slate-800">
              <h4 className="text-xs font-bold text-rose-600 dark:text-rose-400 uppercase tracking-wider flex items-center gap-1.5">
                <Sparkles className="w-4 h-4" />
                <span>4. Kustomisasi Teks Badges & Label Header/Homepage/Footer</span>
              </h4>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">
                    Badge Header (header_badge_text)
                  </label>
                  <input
                    type="text"
                    value={cfgHeaderBadgeText}
                    onChange={(e) => setCfgHeaderBadgeText(e.target.value)}
                    placeholder="misal: Cloudflare D1 Edge Engine"
                    className="w-full px-3.5 py-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-xs font-semibold focus:ring-2 focus:ring-rose-500"
                  />
                </div>
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">
                    Badge Top Hero Banner (hero_badge_text)
                  </label>
                  <input
                    type="text"
                    value={cfgHeroBadgeText}
                    onChange={(e) => setCfgHeroBadgeText(e.target.value)}
                    placeholder="misal: Cloudflare D1 Edge Architecture • TTFB < 20ms"
                    className="w-full px-3.5 py-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-xs font-semibold focus:ring-2 focus:ring-rose-500"
                  />
                </div>
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">
                    Judul Ticker Auto-Links (autolink_ticker_label)
                  </label>
                  <input
                    type="text"
                    value={cfgAutolinkTickerLabel}
                    onChange={(e) => setCfgAutolinkTickerLabel(e.target.value)}
                    placeholder="misal: Kata Kunci SEO Terpopuler:"
                    className="w-full px-3.5 py-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-xs font-semibold focus:ring-2 focus:ring-rose-500"
                  />
                </div>
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">
                    Label Footer Item Auto-Link (footer_autolink_label)
                  </label>
                  <input
                    type="text"
                    value={cfgFooterAutolinkLabel}
                    onChange={(e) => setCfgFooterAutolinkLabel(e.target.value)}
                    placeholder="misal: Auto-Linking Engine On-Page"
                    className="w-full px-3.5 py-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-xs font-semibold focus:ring-2 focus:ring-rose-500"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">
                    Footer Badge 1 (footer_badge_1)
                  </label>
                  <input
                    type="text"
                    value={cfgFooterBadge1}
                    onChange={(e) => setCfgFooterBadge1(e.target.value)}
                    placeholder="Cloudflare Pages Edge"
                    className="w-full px-3.5 py-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-xs font-semibold focus:ring-2 focus:ring-rose-500"
                  />
                </div>
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">
                    Footer Badge 2 (footer_badge_2)
                  </label>
                  <input
                    type="text"
                    value={cfgFooterBadge2}
                    onChange={(e) => setCfgFooterBadge2(e.target.value)}
                    placeholder="Cloudflare D1 SQLite"
                    className="w-full px-3.5 py-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-xs font-semibold focus:ring-2 focus:ring-rose-500"
                  />
                </div>
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">
                    Footer Badge 3 (footer_badge_3)
                  </label>
                  <input
                    type="text"
                    value={cfgFooterBadge3}
                    onChange={(e) => setCfgFooterBadge3(e.target.value)}
                    placeholder="GitHub REST Storage"
                    className="w-full px-3.5 py-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-xs font-semibold focus:ring-2 focus:ring-rose-500"
                  />
                </div>
              </div>
            ...
            <div className="space-y-4 pt-4 border-t border-slate-100 dark:border-slate-800">
              <h4 className="text-xs font-bold text-rose-600 dark:text-rose-400 uppercase tracking-wider flex items-center gap-1.5">
                ...
                <span>5. Footer & Social Media Links</span>
              </h4>
